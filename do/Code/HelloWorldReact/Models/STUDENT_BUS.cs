...
namespace IS.uni
{
    public class STUDENT_BUS
    {
        ...
        public STUDENT_OBJ createObject()
        {
            STUDENT_OBJ obj = new STUDENT_OBJ();
            return obj;
        }
        public STUDENT_OBJ createNull()
        {
            return null;
        }
        public List<STUDENT_OBJ> getAll(params spParam[] listFilter)
        {
            List<STUDENT_OBJ> lidata = new List<uni.STUDENT_OBJ>();
            ...
            else
            {
                foreach (DataRow dr in ds.Tables["Tmp"].Rows)
                {
                    ...
                    Type myTableObject = typeof(STUDENT_OBJ);
                    System.Reflection.PropertyInfo[] selectFieldInfo = myTableObject.GetProperties();

                    Type myObjectType = typeof(STUDENT_OBJ.BusinessObjectID);
                    ...
                    foreach (System.Reflection.PropertyInfo info in selectFieldInfo)
                    {
                        ...
                        {
                            //set id value
                            STUDENT_OBJ.BusinessObjectID objid;
                            objid = (STUDENT_OBJ.BusinessObjectID)info.GetValue(obj, null);
        ...
        public STUDENT_OBJ GetByID(STUDENT_OBJ.BusinessObjectID id)
        {
            List<STUDENT_OBJ> li = getAll(new spParam("CODE", SqlDbType.VarChar, id.CODE, 0));
        ...
        public string genNextCode(STUDENT_OBJ obj)
        {
        ...
        public int Update(STUDENT_OBJ obj)
        {
            int ret = 0;
            DBBase db = new DBBase(ConfigurationSettings.AppSettings["connectionString"].ToString());
        ...
        public int Delete(STUDENT_OBJ.BusinessObjectID obj)
        {
            int ret = 0;
            DBBase db = new DBBase(ConfigurationSettings.AppSettings["connectionString"].ToString());
